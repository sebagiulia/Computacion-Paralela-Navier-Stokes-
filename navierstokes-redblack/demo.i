...
# 123 "demo.c"
  ...
  for ( i=1 ; i<=N ; i++ ) {
   ...
   for ( j=1 ; j<=N ; j++ ) {
    ...
    glVertex2f ( x, y );
    glVertex2f ( x+u[(rb_idx((i),(j),(N+2), (N+2)))], y+v[(rb_idx((i),(j),(N+2), (N+2)))] );
   }
  }
...
# 145 "demo.c"
  ...
  for ( i=0 ; i<=N ; i++ ) {
   ...
   for ( j=0 ; j<=N ; j++ ) {
    ...
    d00 = dens[(rb_idx((i),(j),(N+2), (N+2)))];
    d01 = dens[(rb_idx((i),(j+1),(N+2), (N+2)))];
    d10 = dens[(rb_idx((i+1),(j),(N+2), (N+2)))];
    d11 = dens[(rb_idx((i+1),(j+1),(N+2), (N+2)))];
...
static void react ( float * d, float * u, float * v, unsigned int h, unsigned int w )
{
 ...
 if (max_velocity2<0.0000005f) {
  u[(rb_idx((N/2),(N/2),(N+2), (N+2)))] = force * 10.0f;
  v[(rb_idx((N/2),(N/2),(N+2), (N+2)))] = force * 10.0f;
 }
 if (max_density<1.0f) {
  d[(rb_idx((N/2),(N/2),(N+2), (N+2)))] = source * 10.0f;
 }
 ...
 if ( mouse_down[0] ) {
  u[(rb_idx((i),(j),(N+2), (N+2)))] = force * (mx-omx);
  v[(rb_idx((i),(j),(N+2), (N+2)))] = force * (omy-my);
 }

 if ( mouse_down[2] ) {
  d[(rb_idx((i),(j),(N+2), (N+2)))] = source;
 }
...
#pragma omp parallel
{
        ...
        int block_height = (N + 2) / num_threads;
